  ...
  Shared.mount(root, {
    ...
    data: function () {
      return {
        ...
        lang: Shared.getLang(),
        statusType: 'info',
        statusTitle: 'Ready. This tool runs locally in your browser.',
    ...
    template: '<ymir-tool-frame class="ymir-vue-app--textref" :tool="c" :lang="lang" :status-type="statusType" :status-title="statusTitle" @update-lang="setLang">\
      <template #body>\
        <div v-if="c.kind === \'text\'" class="ymir-vue-textref-grid">\
          <ymir-editor-panel title="Input" :meta="inputMeta" v-model="input" :rows="15"></ymir-editor-panel>\
          <ymir-editor-panel title="Output" :meta="outputMeta" v-model="output" :readonly="true" :rows="15"></ymir-editor-panel>\
        </div>\
        <div v-else-if="c.kind === \'table\'" class="ymir-vue-reference-layout">\
          <div class="ymir-vue-reference-controls"><el-input v-model="query" clearable placeholder="Search this table" style="max-width:420px"></el-input><el-tag>{{ filteredRows.length }} rows</el-tag></div>\
          <div class="ymir-vue-reference-table"><el-table :data="filteredRows" height="520" stripe><el-table-column v-for="col in tableConfig.columns" :key="col.prop" :prop="col.prop" :label="col.label" show-overflow-tooltip></el-table-column></el-table></div>\
        </div>\
        <div v-else-if="c.kind === \'symbols\'" class="ymir-vue-reference-layout">\
          <div class="ymir-vue-reference-controls"><el-input v-model="query" clearable placeholder="Search symbols or category" style="max-width:360px"></el-input><el-select v-model="symbolGroup" style="max-width:240px"><el-option v-for="g in symbolGroups" :key="g" :label="g" :value="g"></el-option></el-select><el-tag>{{ filteredSymbols.length }} symbols</el-tag></div>\
          <div class="ymir-vue-symbol-grid"><button v-for="s in filteredSymbols" :key="s.group+s.symbol" class="ymir-vue-symbol-btn" @click="copySymbol(s.symbol)" :title="s.group">{{ s.symbol }}</button></div>\
        </div>\
        <div v-else-if="c.kind === \'clock\'" class="ymir-vue-clock-grid">\
          <div v-for="row in (c.clockMode === \'world\' ? worldClockRows : localClockRows)" :key="row.zone" class="ymir-vue-clock-card"><strong>{{ row.city }}</strong><span>{{ row.zone }}</span><div class="ymir-vue-clock-time">{{ row.time }}</div><p>{{ row.date }} {{ row.offset }}</p></div>\
        </div>\
        <div v-else-if="c.kind === \'interest\'" class="ymir-vue-calculator-grid">\
          <el-card shadow="never"><el-form label-position="top"><el-form-item label="Principal"><el-input-number v-model="principal" :min="0" :step="100" style="width:100%"></el-input-number></el-form-item><el-form-item label="Annual rate (%)"><el-input-number v-model="annualRate" :min="0" :step="0.1" style="width:100%"></el-input-number></el-form-item><el-form-item label="Years"><el-input-number v-model="years" :min="0" :step="0.25" style="width:100%"></el-input-number></el-form-item><el-form-item label="Compound frequency per year"><el-input-number v-model="compoundFrequency" :min="1" :max="365" style="width:100%"></el-input-number></el-form-item></el-form></el-card>\
          <el-card shadow="never"><div class="ymir-vue-result-stack"><div v-for="row in interestRows" class="ymir-vue-result-line" :key="row.label"><span>{{ row.label }}</span><b>{{ row.value }}</b></div></div></el-card>\
        </div>\
        <div v-else-if="c.kind === \'subnet\'" class="ymir-vue-calculator-grid">\
          <el-card shadow="never"><el-form label-position="top"><el-form-item label="IPv4 address"><el-input v-model="ip"></el-input></el-form-item><el-form-item label="CIDR prefix"><el-input-number v-model="cidr" :min="0" :max="32" style="width:100%"></el-input-number></el-form-item></el-form></el-card>\
          <el-card shadow="never"><div class="ymir-vue-result-stack"><div v-for="row in subnetRows" class="ymir-vue-result-line" :key="row.label"><span>{{ row.label }}</span><b>{{ row.value }}</b></div></div></el-card>\
        </div>\
        <div v-else-if="c.kind === \'palette\'" class="ymir-vue-calculator-grid">\
          <el-card shadow="never"><el-form label-position="top"><el-form-item label="HEX color"><el-input v-model="hex"></el-input></el-form-item></el-form><div class="ymir-vue-color-preview" :style="{background: colorInfo.hex}"></div></el-card>\
          <el-card shadow="never"><div class="ymir-vue-result-stack"><div class="ymir-vue-result-line"><span>HEX</span><b>{{ colorInfo.hex }}</b></div><div class="ymir-vue-result-line"><span>RGB</span><b>{{ colorInfo.rgb }}</b></div><div class="ymir-vue-result-line"><span>HSL</span><b>{{ colorInfo.hsl }}</b></div></div><div class="ymir-vue-palette-grid"><button class="ymir-vue-palette-chip" v-for="c in palette" :key="c" @click="copyPaletteColor(c)"><div class="ymir-vue-palette-swatch" :style="{background:c}"></div><div class="ymir-vue-palette-label">{{ c }}</div></button></div></el-card>\
        </div>\
      </template>\
      <template #actions><ymir-action-buttons :actions="actionItems" @run="run"></ymir-action-buttons></template>\
      <template #footer><el-tag>Shared components</el-tag><el-tag>Vue 3</el-tag><el-tag>Local browser processing</el-tag></template>\
    </ymir-tool-frame>'
  });
})();
